...
`thai-docx/` หาแอปที่ใช้อยู่ด้านล่าง ทำตามขั้นตอน แล้วเริ่มขอไฟล์แรก
...
| สร้างไฟล์แล้ว | ลองแล้ว | ลองแล้ว | ยัง | ยัง | ยัง | ยัง | ลองแล้ว |
...
`.json` มา ให้เก็บไฟล์นี้ไว้ แล้วแนบทุกครั้งที่ต้องการใช้

## แอป Claude

ใช้กับ Claude บนเว็บ (claude.ai) และแอป Claude บนคอมพิวเตอร์ สกิลที่อัปโหลดจะอยู่กับบัญชี Claude ที่ใช้อยู่
...
- หากจะใช้ไฟล์ Markdown รูป หรือไฟล์ Word ที่มีอยู่ ให้แนบในแชต
...
หากต้องการใช้เฉพาะโปรเจกต์เดียว ให้แตกไฟล์ไว้ในโฟลเดอร์ `.claude/skills` ของโปรเจกต์นั้นแทน

**เริ่มใช้**

1. เปิด Claude Code ในโฟลเดอร์ที่ต้องการให้ไฟล์ไปอยู่ ถ้าเปิดค้างไว้ก่อนสร้างโฟลเดอร์สกิล ให้ปิดแล้วเปิดใหม่
...
- โปรไฟล์เก็บที่ `~/.thai-docx/profiles/` ใช้ได้ทุกโปรเจกต์ ถ้าเปิดโหมด sandbox ไว้
  Claude Code อาจเขียนที่นั่นไม่ได้ ให้สั่งว่า "บันทึกไว้ในโปรเจกต์นี้" แทน
...
วิธีใช้ขึ้นกับแพ็กเกจที่ใช้อยู่
...
2. คัดลอกใบสั่งงานจาก [PROMPT.th.md](https://github.com/sayam/thai-docx-skill/blob/main/PROMPT.th.md) มาวาง แล้วพิมพ์คำขอต่อท้าย

วิธีนี้ต้องใช้แชตที่รัน Python กับไฟล์ที่แนบได้ ยังไม่ได้ลองกับแอปใด หากลองแล้วได้หรือไม่ได้ ช่วยแจ้งที่หน้า
[Issues](https://github.com/sayam/thai-docx-skill/issues) ว่าใช้แอปอะไร และเจออะไร

**ข้อควรรู้**

- แนบไฟล์ Markdown รูป หรือไฟล์ Word ที่ต้องการใช้ในแชต
...
- **ให้ใช้ได้ทุกโปรเจกต์:** แตกไฟล์ zip ไว้ใน `~/.copilot/skills` ให้ได้
...
- Cursor อ่าน `~/.claude/skills` ด้วย ถ้าติดตั้ง thai-docx ให้ Claude Code ไว้แล้ว ก็ใช้ได้ทันที
...
| แอป | ให้ใช้ทุกโปรเจกต์ | ให้โปรเจกต์เดียวใช้ |
|---|---|---|
...
หรือใช้ตัวติดตั้งวางให้ ตัวติดตั้งต้องใช้ Node.js จะถามว่าใช้แอปไหนบ้าง และส่งข้อมูลการใช้งานแบบไม่ระบุตัวตน
...
จากนั้นขอไฟล์ Word ได้ทันที หรือพิมพ์ `/thai-docx` ในแอปที่มีคำสั่งขึ้นต้นด้วย `/`
...
สำหรับโปรแกรมที่พัฒนาเอง สกิลจะรันใน container ของผู้ให้บริการ ซึ่งไม่มีอินเทอร์เน็ตและติดตั้งอะไรไม่ได้
...
ขั้นตอนนี้ไม่บังคับ หากมี [GitHub CLI](https://cli.github.com/) ตรวจได้ว่าไฟล์ zip เป็นไฟล์ที่ระบบ
...
`thai-docx-0.2.2.intoto.jsonl` มาด้วย ให้ดาวน์โหลดไฟล์นั้นไว้ข้าง ๆ zip แล้วตรวจกับไฟล์นั้นแทน
...
**ตัวติดตั้งสองตัวให้ของคนละอย่าง**
...
| ขอรุ่นเจาะจง | `gh skill install … --pin v0.2.2` | — |
...
ไม่มีตัวไหนผิด หากต้องการรุ่นที่โปรเจกต์นี้ทดสอบและเขียนบันทึกไว้ ให้เอาจาก release — ด้วย
`gh skill install` หรือดาวน์โหลด zip เอง หากต้องการสิ่งที่อยู่บน `main` ในวันนี้ ให้เลือกทางนั้น
แล้วคาดไว้ว่าหน้าอ้างอิงจะใหม่กว่า release ล่าสุด

การเขียน SKILL.md ใหม่คือเหตุผลที่ค่า checksum ของไฟล์ที่ติดตั้งแล้วจะไม่ตรงกับไฟล์ใน release
ส่วนบรรทัด `version` ที่คู่มือบอกให้ดู ยังบอกค่าเดิม
...
- **ถ้าเปิด repo ที่ clone ไว้เป็นโปรเจกต์ เครื่องมือจะอ่านจาก repo นั้นก่อน** ไม่ว่าจะเก่าเพียงใด
  ให้สั่ง `git pull` ในโฟลเดอร์นั้น

**ลบสกิล**

ลบโฟลเดอร์ `thai-docx` หรือในแอป Claude กด **⋮ > Remove** เมนู **⋮** ในรายการสกิลมีเพียง **Turn off**
(ปิดการใช้งานไว้ ยังไม่ลบ) กับ **Remove** ส่วนเมนูในหน้าของสกิลมีครบกว่า รวมถึง **Download** ที่ขอไฟล์ zip คืนได้
